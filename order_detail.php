...
$totalItemRows = 0;
seller_order_detail_debug('seller_item_filter_context', [
    'sellerUserId' => $sellerUserId,
    'currentSellerId' => $currentSellerId,
]);
foreach ($allOrderRows as $itemRow) {
    ...
    $sellerUserIdCandidate = $sellerUserId > 0 ? $sellerUserId : null;
    $currentSellerIdCandidate = $currentSellerId > 0 ? $currentSellerId : null;
    $candidateOwnerIds = [];
    if ($sellerUserIdCandidate !== null) {
        $candidateOwnerIds[$sellerUserIdCandidate] = true;
    }
    if ($currentSellerIdCandidate !== null) {
        $candidateOwnerIds[$currentSellerIdCandidate] = true;
    }

    $fieldMatches = [];
    $fieldValues = [];
    $numericOwnershipPresent = false;

    $sellerUserOwnerFields = ['seller_user_id', 'owner_user_id', 'user_id'];
    foreach ($sellerUserOwnerFields as $fieldKey) {
        $numericValue = array_key_exists($fieldKey, $itemRow) ? $toPositiveIntOrNull($itemRow[$fieldKey]) : null;
        $fieldValues[$fieldKey] = $numericValue;
        if ($numericValue !== null) {
            $numericOwnershipPresent = true;
            $observedOwnerIds[$numericValue] = true;
            $fieldMatches[$fieldKey] = $sellerUserIdCandidate !== null && $numericValue === $sellerUserIdCandidate;
        } else {
            $fieldMatches[$fieldKey] = false;
        }
    }

    foreach (['seller_id', 'listing_seller_id'] as $fieldKey) {
        $numericValue = array_key_exists($fieldKey, $itemRow) ? $toPositiveIntOrNull($itemRow[$fieldKey]) : null;
        $fieldValues[$fieldKey] = $numericValue;
        if ($numericValue !== null) {
            $numericOwnershipPresent = true;
            $observedOwnerIds[$numericValue] = true;
            $fieldMatches[$fieldKey] = isset($candidateOwnerIds[$numericValue]);
        } else {
            $fieldMatches[$fieldKey] = false;
        }
    }

    $matched = in_array(true, $fieldMatches, true);
    if ($numericOwnershipPresent) {
        $rowsWithNumericOwner++;
    }
    if ($numericOwnershipPresent && !$matched) {
        seller_order_detail_debug('seller_item_filter_row', [
            'order_item_id' => isset($itemRow['item_id']) && is_numeric($itemRow['item_id']) ? (int)$itemRow['item_id'] : null,
            'listing_id' => isset($itemRow['listing_id']) && is_numeric($itemRow['listing_id']) ? (int)$itemRow['listing_id'] : null,
            'seller_user_id' => $fieldValues['seller_user_id'] ?? null,
            'seller_id' => $fieldValues['seller_id'] ?? null,
            'owner_user_id' => $fieldValues['owner_user_id'] ?? null,
            'user_id' => $fieldValues['user_id'] ?? null,
            'listing_seller_id' => $fieldValues['listing_seller_id'] ?? null,
            'matched' => false,
        ]);
        continue;
    }
